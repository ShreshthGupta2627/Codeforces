...
void solve()
{
    ...
    // x ki power nikaalo max upar se !!
    ...
    // Smallest is GCD ... but x times it will increae to curr+1
    int curr = sum - v[n - 1];

    while (freq[curr] % x == 0)
    {
        freq[curr + 1] += freq[curr] / x;
        freq[curr] = freq[curr] % x;
        curr++;
    }

    int ans = 0;
    int MOD = 1e9 + 7;
    ans = mod_pow(x, min(curr, sum), MOD) % MOD;
    cout << ans << endl;
}
int32_t main()
{
